...
| `tools/study.js` | 152 | `studyTopLPers` → Agent Meridian `/top-lp` + `/study-top-lp`. Returns ranked LPer patterns (avg hold, win rate, preferred strategy). **Auto-grows smart-wallets.json post-deploy via `autoGrowSmartWalletsFromStudy` (index.js:1677) with quality gate: closed ≥100, age ≤24h, win >60%, daily cap 5.** |
...
| `decision-log.js` | 68 | Rolling 100-entry log. Types: `deploy` / `close` / `skip` / `no_deploy` / **`smart_wallet_autoadd`** (added 2026-06-22). Each entry: actor, pool, summary, reason, risks[], metrics{}, rejected[]. Surfaced via `get_recent_decisions` tool and `getDecisionSummary()` in the prompt. |
...
| `smart-wallets.js` | 103 | Tracked KOL/alpha wallets. `type: "lp"` (default) checks positions; `type: "holder"` only checks token holdings. 5-min position cache. `check_smart_wallets_on_pool` is the deployment confidence signal. **Auto-grown by `autoGrowSmartWalletsFromStudy` (index.js:1677) from study_top_lpers after successful deploy — gated on closed≥100, age≤24h, win>60%.** |
...
| **Integrations** | | |
| `telegram.js` | 519 | `startPolling(onMessage)`, `stopPolling()`. Long-poll with 35s abort. `createLiveMessage` returns a handle with `toolStart/toolFinish/note/finalize/fail` for live progress. Sends deploy/close/swap/OOR notifications. Auth: `isAuthorizedIncomingMessage` (chatId match + group→allowed user IDs). Registers `/help` `/status` `/positions` `/close` `/closeall` `/set` `/settings` `/setcfg` `/screen` `/candidates` `/deploy` `/briefing` `/hive` `/pause` `/resume` `/stop` via `setMyCommands`. **HTML escaping (2026-06-22)**: `escapeHtml()` helper escapes `& < > "` for any text passed into `sendHTML()`. All 4 notify helpers use it on dynamic fields. **Mandatory for any future `sendHTML` callsite with LLM/user text.** |
...
| `briefing.js` | 78 | HTML daily report. 24h activity, performance, lessons, current portfolio. Sent at 1:00 UTC. **Lesson rules escaped via `safeRule()` (2026-06-22 fix)** — LLM rules can contain `<`, `>`, `&` which break Telegram HTML parse. |
...
- **Telegram `parse_mode: "HTML"` requires escaping** (2026-06-22 fix) — any `<`, `>`, `&`, `"` in dynamic text (LLM output, user input, on-chain data) will trigger `sendMessage 400 "can't parse entities"`. **All `sendHTML()` callsites** escape dynamic fields via `escapeHtml()` in `telegram.js`. **All LLM-generated text joined into briefing** is escaped via `safeRule()` in `briefing.js`. **Rule for future code**: any new `sendHTML()` callsite MUST escape every dynamic field with `escapeHtml()`. The original bug came from a lesson rule containing `"<= -15%"` — Telegram parsed `<=` as a tag with name `=`.
