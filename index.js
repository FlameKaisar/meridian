...
// Trailing-drop confirmation (Jun 24 patch) runs parallel to upstream deterministic exit signaling
// via registerExitSignal/confirmPeak (state.js, e559081). Both can fire; auto-grow daily cap is local-only.
let _pollTriggeredAt = 0;                                                   // cooldown for poller-triggered management
let _lastDeployArgs = null;                                                 // captured deploy_position args
let _smartWalletAddsToday = { date: "", count: 0 };                         // daily cap counter (midnight UTC)
const _peakConfirmTimers = new Map();                                       // upstream peak confirm (legacy path)
const _trailingDropConfirmTimers = new Map();                               // Jun-24 trailing drop
